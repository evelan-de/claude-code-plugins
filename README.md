...
| `/evelan:mission-control` | The control surface for that script from any Claude session: status, add, retry, stop, log, start now, doctor, schedule. Runs the command over SSH on the machine named in `~/.claude/mission-control/host` when that is not the current one, and relays the script's lines as they are. | shipped |
...
**Mission control.** The queue script is `mission-control` (plugin `bin/`). Setup once on the office Mini: `mission-control doctor` (login, `gh`, labels), `~/.claude/mission-control/repos.txt` with one repo path per line, `~/.claude/mission-control/env` (mode 600) with `SLACK_WEBHOOK_URL`, then `mission-control install-schedule 22:00`. On the MacBook write `office-mini` into `~/.claude/mission-control/host`; `/evelan:mission-control` then runs every command there over SSH ("was läuft gerade", "nimm X dazu", "stopp die Warteschlange", "starte jetzt"). Commands and files: `skills/autopilot/references/mission-control.md`.
