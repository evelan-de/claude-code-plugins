...
- Optional model choice - say "mit Sol" / "nutze Luna" / "mit Terra" and the slug is resolved and validated against the live Codex catalog before the run (hidden catalog entries are never offered)
...
- Optional model choice - name a model ("nutze Luna", "mit Sol", "mit Terra") and it is resolved to a real slug and validated against the live Codex catalog before the run, instead of failing with a backend 400 minutes in
