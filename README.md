...
**Ticket updates:** done by the runner, not by the model: before a run `jira start <KEY>` (In Progress, assigned to the token owner), after it `jira comment <KEY> -` with the status, the PR link and the report head; never a done or merged status. The `jira` script (plugin `bin/`; `jira setup | doctor | view | start | comment | transition | assign`) reads `~/.claude/jira/env` (mode 600: `JIRA_SITE`, `JIRA_EMAIL`, `JIRA_TOKEN`; optional `JIRA_START_STATUS`, default `In Progress|In Arbeit`). `jira setup` asks for site, email and an Atlassian API token (hidden input) on every machine that runs the queue, writes the file and runs `doctor`. Without the file the runner logs that the ticket was not updated (`jira-failed` in `done.txt` when a call failed). Python 3, standard library only; the token stays in memory and never reaches a command line, a temp file or the output.
...
- Run: `/autopilot docs/autopilot/sessions/<date>-<slug>` enqueues and starts the runner; the runner launches `claude -p` with `--model` and `--effort` from the plan header (`Model: sonnet` or `opus`; Sonnet always at effort `xhigh` or `max`), `--advisor fable`, `--fallback-model opus` for Sonnet runs, and a budget of 100 USD per attempt for Sonnet, 120 for Opus. `/autopilot <session dir> opus high` switches an existing plan to Opus and/or sets its effort before it is queued (model and effort are both optional). A ticket key also works when its plan exists. Without a plan the run writes one itself with conservative decisions and no questions.
...
- Every review-bot comment gets a reply in its thread from the run: `Fixed in <sha>: ...` or `Not changed: <reason>`; the runner reports threads left without one (`unanswered-review-comments=N`).
- A run never ends its turn to wait (headless: the turn end is the process end). Codex review, CI watch and review-bot polling run in the foreground; the `Stop` hook refuses a turn end without `REPORT.md` or `HANDOFF.md`, and a done report without a `## Review bot` section in a project with the review workflow. A run that still exits early is restarted by the runner like a hand-off.
