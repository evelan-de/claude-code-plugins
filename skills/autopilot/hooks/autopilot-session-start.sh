...
echo "Context was compacted. If you are running an autopilot session, its artifacts are in $dir: re-read PLAN.md (package statuses, decisions, goal artifact) and, if present, HANDOFF.md before continuing. The gate command is in .claude/autopilot.json; the gate evidence log is .claude/autopilot-gate.log. Do not re-explore the repo: DIGEST.md in the session folder is the digest."
exit 0
