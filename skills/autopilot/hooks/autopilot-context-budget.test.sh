...
hook_input() {
  # $1 transcript  $2 session  $3 agent (optional)
  if [ -n "${3:-}" ]; then
    jq -n --arg t "$1" --arg s "$2" --arg a "$3" '{tool_name:"Bash",transcript_path:$t,session_id:$s,agent_id:$a,hook_event_name:"PostToolUse"}'
  else
...
# 5. a different agent_id in the same session has its own counter
out="$(hook_input "$T" s3 agentB | CLAUDE_PROJECT_DIR="$P" bash "$HOOK")"
printf '%s' "$out" | jq -e '.hookSpecificOutput.additionalContext' >/dev/null 2>&1 && ok "separate counter per agent" || fail "per-agent counter (got $out)"
...
# 7. env override wins
out="$(hook_input "$T" s5 | AUTOPILOT_CONTEXT_BUDGET=200000 CLAUDE_PROJECT_DIR="$P" bash "$HOOK")"
