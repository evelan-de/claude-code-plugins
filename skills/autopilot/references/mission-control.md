...
| `host` | Not read by the script. One hostname (`office-mini`); the `/mission-control` skill runs every command over SSH on that machine when it is not the current one. |
...
mission-control stop                      # TERM the active run (its trap kills claude), wait up to 15 s
mission-control retry <repo> <#pr | item> # PR: label autopilot-blocked -> autopilot-ready; item: add it again
mission-control log [<item substring>]    # last 40 lines of the newest item log (or the newest matching one)
mission-control install-schedule 22:00    # LaunchAgent, daily at that time (macOS)
mission-control uninstall-schedule
mission-control kickstart                 # run the installed LaunchAgent now, inside the GUI session
```
...
claude process (SIGTERM, SIGKILL after 10 s) and releases the lock. Prints `stopped`, or
`still running (pid N)` with exit 1 after 15 s, or `nothing running`. The stopped item stays
in `queue.txt` (a list item) or keeps its label (a PR item), so the next run takes it again.

`retry <repo> <#pr>` (a `#` prefix or a bare number) swaps `autopilot-blocked` for
`autopilot-ready` on that PR and says so; `retry <repo> <item>` appends the item to
`queue.txt` like `add`, with the branch the kept worktree is on when the item is a session
directory, so the retry continues where the blocked run stopped.

`log` names the file it shows (`log: <path>`), then `tail -n 40` of it. Item logs only;
`queue.log` and `launchd.log` are read directly.

`kickstart` runs `launchctl kickstart -k gui/<uid>/de.evelan.mission-control`: the installed
LaunchAgent starts a `run` now, inside the GUI session, so the Keychain login is readable
even when the command arrives over SSH. Errors when no schedule is installed. Output goes to
`logs/launchd.log`; follow it with `log` once an item log exists.
